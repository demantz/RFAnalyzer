 ...
 */
public AudioSink(int packetSize, @SuppressWarnings("SameParameterValue") int sampleRate) {
	thread = new Thread(this, "AudioSink Thread");
	Log.v(LOGTAG, "constructor: packetSize: " + packetSize + ", sampleRate: " + sampleRate);
	this.packetSize = packetSize;
	this.sampleRate = sampleRate;

	// Create the queues and fill them with
	this.inputQueue = new ArrayBlockingQueue<>(QUEUE_SIZE);
	this.outputQueue = new ArrayBlockingQueue<>(QUEUE_SIZE);
...
public void run() {
	...
	Log.i(LOGTAG, "AudioSink started. (Thread: " + thread.getName() + ")");
	...
	Log.i(LOGTAG, "AudioSink stopped. (Thread: " + thread.getName() + ")");
}
